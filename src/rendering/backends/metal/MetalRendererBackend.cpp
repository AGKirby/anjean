...
namespace Anjean::Rendering {
  ...
  MetalRendererBackend::MetalRendererBackend(Anjean::Window& window)
    ...
    if (!m_impl->residencySet) {
      const char* message = residencyError ? residencyError->localizedDescription()->utf8String()
    ...
    m_impl->timeBuffer = m_impl->device->newBuffer(sizeof(float), MTL::ResourceStorageModeShared);
  ...
  void MetalRendererBackend::beginFrame(const Color& clearColor) {
    ...
    m_impl->elapsedSeconds = std::chrono::duration<float>(now - m_impl->startTime).count();
  ...
  MetalRendererBackend::loadMeshToGPU(Mesh pMesh) {
  ...
  void MetalRendererBackend::draw(const DrawCommand& command) {
    ...
    m_impl->renderEncoder->drawPrimitives(MTL::PrimitiveTypeTriangle, 0, command.mesh.vertexCount,
                                          1);
  ...
  void MetalRendererBackend::drawSprite(const PipelineHandle& pPipeline,
    ...
    MTL::Buffer* objectUniformBuffer = m_impl->device->newBuffer(
        &pObjectUniform, sizeof(ObjectUniformHandle), MTL::ResourceStorageModeShared);
