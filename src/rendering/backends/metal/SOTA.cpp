...
namespace Anjean::Rendering {
  namespace {
    ...
    template <typename T> static bool HasBaseColorTexture(const T& desc) {
    ...
    template <typename T> static bool IsDepthEnabled(const T& desc) {
    ...
    template <typename T> static bool IsBlendingEnabled(const T& desc) {
  ...
  MetalRendererBackend::loadMeshToGPU(Mesh pMesh) {
    ...
    mesh.vertexCount = pMesh.vertexCount != 0
                           ? pMesh.vertexCount
                           : static_cast<decltype(mesh.vertexCount)>(pMesh.vertices.size());
