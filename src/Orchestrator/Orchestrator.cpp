...
namespace Anjean::Orchestrator {
  Orchestrator::Orchestrator() {
    ...
    try {
      auto window =
          new Anjean::Window("Anjean", 1280.0f, 880.0f, SDL_WINDOW_RESIZABLE | SDL_WINDOW_METAL);
    ...
    if (renderer) {
      ...
      for (Runtime::GameObject* gO : gameObjectsToRender) {
        ...
        objectUniformHandle.modelTranslation = {
            ...
            simd_make_float4(gO->transform.position.x, gO->transform.position.y,
        ...
        objectUniformHandle.modelRot = matrix_multiply(
            ...
            matrix_multiply(Rendering::makeRotationY(gO->transform.rotation.y *
                                                     (3.14159265358979323846f / 180.0f)),
                            Rendering::makeRotationX(gO->transform.rotation.x *
                                                     (3.14159265358979323846f / 180.0f))));
        ...
        auto cameraRot = matrix_multiply(
            Rendering::makeRotationZ(currentCamera.transform.rotation.z *
                                     (3.14159265358979323846f / 180.0f)),
            matrix_multiply(Rendering::makeRotationY(currentCamera.transform.rotation.y *
                                                     (3.14159265358979323846f / 180.0f)),
                            Rendering::makeRotationX(currentCamera.transform.rotation.x *
                                                     (3.14159265358979323846f / 180.0f))));
        simd_float4x4 cameraMatrix = matrix_multiply(
            matrix_multiply({simd_make_float4(1, 0, 0, 0), simd_make_float4(0, 1, 0, 0),
                             simd_make_float4(0, 0, 1, 0),
                             simd_make_float4(-currentCamera.transform.position.x,
        ...
        renderer->drawSprite(renderState.spritePipelineHandle, meshData, texture,
                             objectUniformHandle);
      }
      renderer->endFrame();
    }
