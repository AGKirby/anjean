...
namespace Anjean::Runtime {
  namespace {
    ...
    void loadHostFxr() {
      const char* hostfxrPath = "/usr/local/share/dotnet/host/fxr/8.0.1/libhostfxr.dylib";
      ...
      hostfxr_initialize = reinterpret_cast<hostfxr_initialize_for_runtime_config_fn>(
          dlsym(lib, "hostfxr_initialize_for_runtime_config"));

      hostfxr_get_delegate = reinterpret_cast<hostfxr_get_runtime_delegate_fn>(
          dlsym(lib, "hostfxr_get_runtime_delegate"));

      hostfxr_close_ptr = reinterpret_cast<hostfxr_close_fn>(dlsym(lib, "hostfxr_close"));
    ...
    getLoadAssemblyFn(const std::string& runtimeConfigPath) {
      hostfxr_handle context = nullptr;

      int rc = hostfxr_initialize(runtimeConfigPath.c_str(), nullptr, &context);

      if (rc < 0 || context == nullptr) {
        std::cout << "hostfxr init failed. rc = " << rc << ", context = " << context << std::endl;
        throw std::runtime_error("hostfxr_initialize_for_runtime_config failed.");
      }

      void* loadAssembly = nullptr;

      rc = hostfxr_get_delegate(context, hdt_load_assembly_and_get_function_pointer, &loadAssembly);
  ...
  void ScriptingEngine::load(const std::string& runtimeConfigPath,
                             const std::string& scriptingDllPath) {
    loadHostFxr();

    std::cout << runtimeConfigPath << std::endl;

    auto loadAssemblyAndGetFunctionPointer = getLoadAssemblyFn(runtimeConfigPath);

    int rc = 0;

    rc = loadAssemblyAndGetFunctionPointer(
        scriptingDllPath.c_str(), "Anjean.ScriptHost, Anjean.Scripting", "LoadGameAssembly",
        UNMANAGEDCALLERSONLY_METHOD, nullptr, reinterpret_cast<void**>(&loadGameAssemblyFn));
    ...
    rc = loadAssemblyAndGetFunctionPointer(
        scriptingDllPath.c_str(), "Anjean.ScriptHost, Anjean.Scripting", "CreateGameObjectScript",
        UNMANAGEDCALLERSONLY_METHOD, nullptr, reinterpret_cast<void**>(&createGameObjectScriptFn));
    ...
    rc = loadAssemblyAndGetFunctionPointer(
        scriptingDllPath.c_str(), "Anjean.ScriptHost, Anjean.Scripting", "StartMainScene",
        UNMANAGEDCALLERSONLY_METHOD, nullptr, reinterpret_cast<void**>(&startMainSceneFn));
    ...
    rc = loadAssemblyAndGetFunctionPointer(
        scriptingDllPath.c_str(), "Anjean.ScriptHost, Anjean.Scripting", "UpdateAll",
        UNMANAGEDCALLERSONLY_METHOD, nullptr, reinterpret_cast<void**>(&updateAllFn));
    ...
    rc = loadAssemblyAndGetFunctionPointer(
        scriptingDllPath.c_str(), "Anjean.ScriptHost, Anjean.Scripting", "PhysicsUpdateAll",
        UNMANAGEDCALLERSONLY_METHOD, nullptr, reinterpret_cast<void**>(&physicsUpdateAllFn));
  ...
  void ScriptingEngine::createGameObjectScript(const std::string& scriptName,
    ...
    if (rc != 0) {
      throw std::runtime_error("Failed to create script: " + scriptName +
                               ", rc = " + std::to_string(rc));
    }
  }
  ...
  void ScriptingEngine::startMainScene() {
    ...
    if (rc != 0) {
      throw std::runtime_error("Failed to start main scene, rc = " + std::to_string(rc));
    }
  }
} // namespace Anjean::Runtime